...
public class CheckCodeUtil {
	...
	public static void makeCheckCode(HttpServletRequest request, HttpServletResponse response)
		...
		int width=120,height=39;		//指定生成验证码的宽度和高度
		...
		g.setColor(new Color(73,133,19)); //图片颜色
		...
		for(int i=0;i<50;i++){
			...
			BasicStroke bs=new BasicStroke(2f,BasicStroke.CAP_BUTT,BasicStroke.CAP_BUTT);	//定制线条样式
...
//			g.setColor(color);
	...
	public static boolean check(HttpServletRequest request) {
		...
		System.out.println("输入的验证码是：" + input);
		System.out.println("seesion中的验证码是：" + session);
		if (StringUtils.isNotBlank(session) && StringUtils.isNotBlank(input) && session.trim().equalsIgnoreCase(input.trim())) {
			return true;
		}
		return false;
	}
